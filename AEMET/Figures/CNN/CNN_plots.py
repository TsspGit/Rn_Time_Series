...
plt.plot(test_loss, color='k', marker='x', linestyle=':')
plt.ylabel('loss', fontsize=14)
plt.xlabel('epoch', fontsize=14)
plt.legend(['train', 'test'], loc='upper right')
plt.tight_layout()
plt.savefig('./CNN_Loss_Rn_{}_{}_wcolor.eps'.format(neuron[0], neuron[1]))
...
ax.plot(pd.date_range(startdaypred, periods=len(pred), freq='W'), pred, color='k', linestyle=':')
...
axins = zoomed_inset_axes(ax, 1.7, loc='lower left', bbox_to_anchor=(567,140))
...
axins.plot(pd.date_range(startday, periods=len(mdnRnA), freq='W'), mdnRnA, 'k') 
axins.plot(pd.date_range(startdaypred, periods=len(pred), freq='W'), pred, color='k', linestyle=':')
...
fig.savefig('./CNN_Rn_{}_{}_wcolor.eps'.format(neuron[0], neuron[1]), bbox_inches='tight', dpi=300)
...
plt.legend(['train', 'test'], loc='upper right')
plt.tight_layout()
plt.savefig('./CNN_Loss_RnT_PMP_{}_{}_wcolor.eps'.format(neuron[0], neuron[1]))
...
mark_inset(ax, axins, loc1=2, loc2=4, fc="none", ec="0.5")
plt.suptitle('Weekly Fitting at LSC - Hall A')
fig.savefig('./CNN_RnT_PMP_{}_{}_wcolor.eps'.format(neuron[0], neuron[1]), bbox_inches='tight', dpi=300)
plt.show()