...
fig,ax = plt.subplots(nrows=1,ncols=1,figsize=(8,4), constrained_layout=True, dpi=100)
xaxis = ax.get_xaxis()
...
ax.plot(pd.date_range(startdaypred, periods=len(pred), freq='W'), pred, color='dimgray', marker='x', linestyle='--')
...
axins.plot(pd.date_range(startdaypred, periods=len(pred), freq='W'), pred, color='dimgray', marker='x', linestyle='--')
...
fig.savefig('./CNN_Rn_{}_{}_wcolor.eps'.format(neuron[0], neuron[1]), bbox_inches='tight')
