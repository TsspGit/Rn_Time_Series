...
def scaleDF(DF):
    ...
    Tscaled = DF['tmed'].values.reshape(-1, 1)
    outputDF['mdnRnA'] = scalermdnRnA.fit_transform(mdnRnAscaled).reshape(-1)
    outputDF['tmed'] = scalerT.fit_transform(Tscaled).reshape(-1)
...
EAM = []
for it in range(25):
    history, pred, acc_train, acc_test, model = LSTM_v2(neuron, 7, X_train, Y_train, X_test, Y_test, sample_size)
...
# Scale DataFrame
weekly_scaled, scalers = scaleallDF(DF_list, arr_str)
...
#Rn + Temperature
# print('\n\n#########\n Radon + Temperature \n########\n\n')
DFscaled_list, pdatamdnRnAlags = build_pdata(DFscaled_list, DF, nlags, ['tmed', 'mdnRnA'])    # This 2 lines
Xt = data_toCNN_format_v2(DFscaled_list, arr_str, ['tmed', 'mdnRnA'], sample_size, pdatalags) # for LSTM
...
neuron = [52, 104]
show_errors_LSTM(neuron, 7, Xtrainlist, Ytrain, Xtestlist, Ytest, arr_str, iterations=25, sample_size=sample_size, DF_mdnRnA=DF)
